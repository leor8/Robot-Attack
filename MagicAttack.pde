class MagicAttack extends AttackTower {
  int framesMA = 0;
  ArrayList<MagicBullet> bulletsMA = new ArrayList<MagicBullet>();
  PImage Magic;

  MagicAttack(PVector pos, int health, int dmg, int attackSpeed, PVector vel, PVector dim) {
    super(pos, health, dmg, attackSpeed, vel, dim);
    
    Magic = loadImage("MagicAttack.png");
  }

  void fire() {
    bulletsMA.add(new MagicBullet(new PVector(pos.x + 45, pos.y + 15), new PVector(12, 0)));
  }

  void checkProjectiles() {
    for (int i=0; i<bulletsMA.size(); i++) {
      MagicBullet bulletma = bulletsMA.get(i);
      //update the bullet
      bulletma.update();  
      ...
      for (int j = 0; j < be.size(); j++) {
        ...
        if (bulletma.hitCharacter(be1)) {
          bulletma.hitBasicEnemy(be1, dmg);
        }
      }
      // Day 2
      ...
      for (int j = 0; j < fe.size(); j++) {
        ...
        if (bulletma.hitCharacter(fe1)) {
          bulletma.hitFastEnemy(fe1, dmg);
        }
      }
      // Day 2
      ...
      for (int j = 0; j < te.size(); j++) {
        ...
        if (bulletma.hitCharacter(te1)) {
          bulletma.hitTankEnemy(te1, dmg);
        }
      }
      // Day 3
      ...
      for (int j = 0; j < ue.size(); j++) {
        ...
        if (bulletma.hitCharacter(ue1)) {
          bulletma.hitUpdateEnemy(ue1, dmg);
        }
      }
      // Day 4
  ...
  void update() {
    ...
    if (be.size() > 0 || fe.size() > 0 || be2.size() > 0 || fe2.size() > 0 || te.size() > 0 || be3.size() > 0 || be4.size() > 0 || fe3.size() > 0 || fe4.size() > 0 || te2.size() > 0 || te3.size() > 0 || ue.size() > 0 || ue2.size() > 0 || bossAlive) {
      if (framesMA == attackSpeed && ((pos.y == 190 && top) || (pos.y == 350 && middle) || (pos.y == 510 && bottom))) {
        framesMA = 0;
        fire();
      }
    }
  ...
  void render() {
    ...
    scale(0.3);
    image(Magic, -Magic.width/2, -Magic.height/2);
    popMatrix();
  }
