class FastAttack extends AttackTower {
  int framesFA = 0;
  ArrayList<FastBullet> bulletsFA = new ArrayList<FastBullet>();
  
  PImage fast;

  FastAttack(PVector pos, int health, int dmg, int attackSpeed, PVector vel, PVector dim) {
    super(pos, health, dmg, attackSpeed, vel, dim);
    
    fast = loadImage("FastAttack.png");
  }
  void fire() {
      bulletsFA.add(new FastBullet(new PVector(pos.x + 45, pos.y + 15), new PVector(12, 0)));
  }

  void checkProjectiles() {
    for (int i=0; i<bulletsFA.size(); i++) {
      FastBullet bulletfa = bulletsFA.get(i);
      //update the bullet
      bulletfa.update();  
      ...
      for (int j = 0; j < be.size(); j++) {
        ...
        if (bulletfa.hitCharacter(be1)) {
          bulletfa.hitBasicEnemy(be1, dmg);
        }
      }
      // Day 2
      ...
      for (int j = 0; j < fe.size(); j++) {
        ...
        if (bulletfa.hitCharacter(fe1)) {
          bulletfa.hitFastEnemy(fe1, dmg);
        }
      }
      // Day 2
      ...
      for (int j = 0; j < te.size(); j++){
        ...
        if (bulletfa.hitCharacter(te1)) {
          bulletfa.hitTankEnemy(te1, dmg);
        }
      }
      // Day 3
      ...
      for (int j = 0; j < ue.size(); j++){
        ...
        if (bulletfa.hitCharacter(ue1)) {
          bulletfa.hitUpdateEnemy(ue1, dmg);
        }
      }
      // Day 4
  ...
  void update() {
   ...
   if (be.size() > 0 || fe.size() > 0 || be2.size() > 0 || fe2.size() > 0 || te.size() > 0 || be3.size() > 0 || be4.size() > 0 || fe3.size() > 0 || fe4.size() > 0 || te2.size() > 0 || te3.size() > 0 || ue.size() > 0 || ue2.size() > 0 || bossAlive) {
      if (framesFA == attackSpeed && ((pos.y == 190 && top) || (pos.y == 350 && middle) || (pos.y == 510 && bottom))) {
      framesFA = 0;
      fire();
      }
    }
  ...
  void render() {
    ...
    scale(0.3);
    image(fast, -fast.width/2, -fast.height/2);
    popMatrix();
  }
